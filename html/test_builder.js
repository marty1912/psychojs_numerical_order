...
function experimentInit() {
  ...
  fixation_dot = new visual.ImageStim({
    win : psychoJS.window,
    name : 'fixation_dot', units : undefined, 
...
function trialRoutineBegin(trials) {
  return function () {
    ...
    trialComponents.push(fixation_dot);
...
function trialRoutineEachFrame(trials) {
  return function () {
    ...
    if (key_resp.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp.getKeys({keyList: ['j', 'k', 's'], waitRelease: false});
    ...
    if (t >= 1.0 && fixation_dot.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      fixation_dot.tStart = t;  // (not accounting for frame time here)
    ...
    if (fixation_dot.status === PsychoJS.Status.STARTED && t >= frameRemains) {
